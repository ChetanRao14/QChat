              ...
              {callStatus !== 'idle' && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="absolute inset-0 z-50 bg-navy-950/95 backdrop-blur-md flex flex-col">
                  {callStatus === 'receiving' ? (
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center">
                      <div className={`w-24 h-24 rounded-full bg-gradient-to-br ${avatarGrad(peer.username)} flex items-center justify-center font-bold text-4xl text-white mb-6`}>
                        <div className="animate-pulse">{peer.username[0].toUpperCase()}</div>
                      </div>
                      <h2 className="text-2xl font-bold mb-2">Calling {peer.username}...</h2>
                      <p className="text-cyan-400 flex items-center gap-2 mb-8"><Lock size={14}/> ML-KEM-768 Secured</p>
                      <div className="relative w-32 h-24 mb-4 rounded-xl overflow-hidden border border-white/10 shadow-2xl">
                        <video ref={localVideoRef} autoPlay playsInline muted className="w-full h-full object-cover" />
                      </div>
                      <button onClick={endCall} className="w-16 h-16 rounded-full bg-rose-500 hover:bg-rose-600 flex items-center justify-center shadow-lg shadow-rose-500/30 transition-colors" aria-label="End call" title="End call">
                        <PhoneOff size={28} className="text-white"/>
                      </button>
                      <span className="mt-3 text-sm text-slate-400">End Call</span>
                    </div>
                  ) : (
                    <>
                      {/* Video area */}
                      <div className="relative flex-1 min-h-0 bg-black overflow-hidden">
                        ...
                        <div className="absolute top-4 right-4 w-32 md:w-48 aspect-[3/4] bg-navy-900 rounded-xl overflow-hidden shadow-2xl border border-white/10 z-10">
                          <video ref={localVideoRef} autoPlay playsInline muted className="w-full h-full object-cover" />
                        </div>
                        {mediaProtection && (
                          <div className={`absolute top-4 left-4 z-10 flex items-center gap-2 px-3 py-1.5 rounded-full text-[11px] font-semibold backdrop-blur-md border ${
                            mediaProtection === 'pq'
                      ...
                      <div className="flex-shrink-0 flex items-center justify-center gap-4 bg-navy-900/95 backdrop-blur-lg px-6 py-4 border-t border-white/10">
                        <button onClick={toggleMute} className={`p-3 rounded-full transition-colors ${isMuted ? 'bg-rose-500/20 text-rose-500' : 'bg-white/10 text-white hover:bg-white/20'}`} title={isMuted ? 'Unmute' : 'Mute'}>
                          {isMuted ? <MicOff size={20}/> : <Mic size={20}/>}
                        </button>
                        <button onClick={toggleVideo} className={`p-3 rounded-full transition-colors ${isVideoOff ? 'bg-rose-500/20 text-rose-500' : 'bg-white/10 text-white hover:bg-white/20'}`} title={isVideoOff ? 'Turn video on' : 'Turn video off'}>
                          {isVideoOff ? <VideoOff size={20}/> : <Video size={20}/>}
                        </button>
                        <button onClick={endCall} className="px-6 py-3 rounded-full bg-rose-500 text-white hover:bg-rose-600 shadow-lg shadow-rose-500/30 flex items-center gap-2 transition-colors" aria-label="End call" title="End call">
                          <PhoneOff size={20}/>
